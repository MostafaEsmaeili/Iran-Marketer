...
{
    public static class IranMarketerCustomUtility
    {
        #region Constants

        private static string _SEP = "-";
        public const string _allTitleFa = " همه";
        public const string _allTitleEn = " All";
        public const string _allValue = "-1";
        #endregion
        ...
        public static int GetDifferenceByDays(DateTime fromDate, DateTime toDate)
        {
            var difference = Math.Abs((fromDate - toDate).Days);
            if (difference > 365)
            {
                difference = 90;
            }
        ...
        //        return IPAddress.Parse(((HttpContextBase)request.Properties["MS_HttpContext"]).Request.UserHostAddress).MapToIPv4().ToString();
        //    }
        //    {
        ...
        public static T ParseEnum<T>(this string value, T defaultValue) where T : struct
        {
            ...
            {
                T enumValue;
                return !Enum.TryParse(value, true, out enumValue) ? defaultValue : enumValue;
            }
            catch (Exception)
            {
                return defaultValue;
        ...
        public static decimal InPercent(this decimal val)
        {
            return val * 100;
        }
        ...
        public static ProductType GetProductTypeByIsin(string isin)
        {
            if (isin.StartsWith("IRBE") || isin.StartsWith("IRBK"))
            {
        ...
        public static SecurityExchange GetSecurityExchange(string isin)
        {
            if (isin.StartsWith("IRT1K"))
            {
                return SecurityExchange.IME;
            }
            switch (isin.Substring(0, 4))
            {
                case "IRR1":
                case "IRO1":
                case "IRB5":
                case "IRB6":
                case "IRT1":
                    return SecurityExchange.TSE;
                case "IRR3":
                case "IRO3":
                case "IRB3":
                case "IRT3":
                    return SecurityExchange.IFB;
                default:
                    return SecurityExchange.TSE;
            }
        }
        ...
        public static DateTime CastToMaxDateTime(this DateTime date)
        {
            var dt = date.SafeDateTime();
            return new DateTime(dt.Year, dt.Month, dt.Day, 23, 59, 59);
        }

        public static string CastToDateWithOutSlash(this DateTime date)
        {
            var dt = date.SafeDateTime();
            return dt.Year + ""  + dt.Month + "" + dt.Day;
        }
        public static string CastToDateWithSlash(this DateTime date)
        {
            var dt = date.SafeDateTime();
            return dt.Year + "/" + dt.Month + "/" + dt.Day;
        }
        ...
        public static int GetPersianDatesDiff(DateTime fromDate, DateTime toDate)
        {
            ...
            var fromIsLeap = calendar.IsLeapYear(calendar.GetYear(fromDate));
            var toIsLeap = calendar.IsLeapYear(calendar.GetYear(toDate));

            DateTime dt1 = calendar.ToDateTime(calendar.GetYear(fromDate), calendar.GetMonth(fromDate), calendar.GetDayOfMonth(fromDate), 0, 0, 0, 0);
            DateTime dt2 = calendar.ToDateTime(calendar.GetYear(toDate), calendar.GetMonth(toDate), calendar.GetDayOfMonth(toDate), 0, 0, 0, 0);
            TimeSpan ts = dt2.Subtract(dt1);
            int days = ts.Days;
            if (fromIsLeap || toIsLeap)
            {
                days = days + 1;
            }
            return days;
        ...
        public static int GetPersianDateDiff(this DateTime fromDate, DateTime toDate)
        {
            if (fromDate <= MinDate || toDate <= MinDate)
            {
                return 0;
    ...
    public static class SerializeDesrialize
    {
