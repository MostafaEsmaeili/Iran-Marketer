...
using CustomExtention = Kendo.Mvc.Extensions.CustomExtention;
        ...
        public ActionResult GetCustomerPayments([DataSourceRequest] DataSourceRequest reques, CustomerPaymentsFilterModel model)
        {
            try
            {
                ...
                var filter = new CustomerPaymentsReportFilter
                {
                    ...
                    {
                        EndDate = Kendo.Mvc.Extensions.CustomExtention.ConvertJalaliToMiladi(model.Date)
                            .GetValueOrDefault(DateTime.Now).RemoveMinutes()
                    },
                ...
                var filter = new CashFlowMasterReportFilter
                {
                    ...
                    DateFilter = new DateFilter
                    {
                        EndDate = CustomExtention.ConvertJalaliToMiladi(model.ToDate)
                            .GetValueOrDefault(DateTime.Now.AddDays(1)).RemoveMinutes(),
                        StartDate = CustomExtention.ConvertJalaliToMiladi(model.FromDate)
                            .GetValueOrDefault(DateTime.Now.AddDays(-15).RemoveMinutes()),
